...
class DDoubleFCNMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(DDoubleFCNMaskHead, self).__init__()
        ...
        self.upsample_1 = build_upsample_layer(upsample_cfg_)
        self.upsample_2 = build_upsample_layer(upsample_cfg_)
    ...
    def init_weights(self):
        for m in [self.upsample_1, self.upsample_2, self.conv_logits]:
            if m is None:
                continue
    ...
    def forward(self, x1, x2):
        for conv in self.convs_1:
            x1 = conv(x1)
        if self.upsample_1 is not None:
        ...
        for conv in self.convs_2:
            x = conv(x)
        if self.upsample_2 is not None:
            x = self.upsample_2(x)
            if self.upsample_method == 'deconv':
                x = self.relu(x)
